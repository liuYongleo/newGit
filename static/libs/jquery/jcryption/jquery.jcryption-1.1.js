...
(function($) {
    ...
    /*
	* 2019.9.26 龚家豪: 将函数改为直接return key , 而不是用回调函数。
	* 修复多次调用传不同data，key始终为第一次的值的问题
    * */

	$.jCryption.getKeys = function(data,callback) {//2015.6.15,将url改为json对象.王琛
		var base = this;
		var testGetKeys = function() {
            var keys = new jCryptionKeyPair(data.e,data.n,data.maxdigits);
			return keys
		};
		function jCryptionKeyPair(encryptionExponent, modulus, maxdigits) {
    ...
    // 2019.9.26 龚家豪: 将函数改为直接return encryptedString, , 而不是用回调函数
